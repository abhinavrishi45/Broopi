    ...
    <div className="p-6 space-y-10">
      <div className="grid grid-cols-1  md:grid-cols-3 ml-12 gap-4">
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          <img onClick={() => navigate('/service/PestControlling')} src={pest} alt="Pest Control" className="w-full h-full object-cover cursor-pointer hover:scale-104 transition-transform duration-300 ease-in-out " />
        </div>

        <div className="bg-yellow-50 rounded-xl shadow-md overflow-hidden ">
          <img onClick={() => navigate('/service/ACService')} src={ac} alt="AC Servicing" className="w-full h-full object-cover hover:scale-104 transition-transform duration-300 ease-in-out cursor-pointer" />
        </div>

        <div className="bg-beige-100 rounded-xl shadow-md overflow-hidden ">
          <img onClick={() => navigate('/service/HomeCleaning')} src={kitchen} alt="Kitchen Cleaning" className="w-full h-full object-cover hover:scale-104 transition-transform duration-300 ease-in-out cursor-pointer" />
        </div>
      </div>
      <div>
        ...
        <div className="grid grid-cols-2 ml-12 md:grid-cols-5 gap-4">
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <img onClick={() => navigate('/service/BathroomCleaning')} src={Carpenter} alt="Carpenter" className="w-full h-60 object-cover hover:scale-102 transition-transform duration-300 ease-in-out cursor-pointer" />
          </div>

          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <img onClick={() => navigate('/service/ACService')} src={acservicing} className="w-full h-60 object-cover hover:scale-102 transition-transform duration-300 ease-in-out cursor-pointer" />
          </div>

          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <img onClick={() => navigate('/service/Electrician')} src={kitchenclean} alt="Kitchen Cleaning" className="w-full h-60 object-cover hover:scale-102 transition-transform duration-300 ease-in-out cursor-pointer" />
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 ml-12 md:grid-cols-5 gap-5">
          ...
          {homecard.map((item) => (
            <div key={item._id}
              onClick={() => navigate(`/service/${item.category}`)} className='bg-gray-100 rounded shadow-md overflow-hidden cursor-pointer hover:scale-102 transition-transform duration-300 ease-in-out  '>
              <img src={`http://localhost:5000/uploads/allproducts/${item.image}`} alt={item.name} className="w-full h-60 object-cover " />
              <p className='text-xl font-bold ml-1'>{item.name}</p>
              <p className='font-semibold ml-1'>{item.category}</p>
              <div className='flex flex-row'>
                <img className='h-6 w-4 ml-1' src={star} alt="star rating" />
                <p>0 (0K)</p>
              </div>
              <p className='font-semibold ml-2'>₹ {item.newPrice}</p>
              <p className='font-semibold line-through ml-3 mb-2'>₹{item.oldPrice}</p>
            </div>
          ))}
      ...
      <div className="bg-white shadow-md ml-12 mt-25 overflow-hidden">
        <img onClick={() => navigate('/service/ACService')} src={ACserviceLong} alt="Home Cleaning" className="w-full h-130  object-cover hover:scale-101 transition-transform duration-300 ease-in-out cursor-pointer" />
      </div>
      <div>
