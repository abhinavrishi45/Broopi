...
const FirstCard = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/firstcard", formData, {
  ...
  const fetchProduct = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/firstcard");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/firstcard/${id}`);
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/firstcard/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      setEditId(null);
      setEditImage(null);
    ...
    <div className="flex flex-col gap-6 p-5 rounded">
      <div className="flex flex-row bg-purple-200 gap-10 p-5 rounded items-center">
        ...
        {preview && (
          <img
            src={preview}
            alt="preview"
            className="w-85 h-50 object-cover rounded-md"
          />
        )}
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {showcard.map((card) => (
          <div
            key={card._id}
            className="bg-white p-2 rounded shadow flex flex-col gap-2 items-center"
          >
            <img
              src={`http://localhost:5000/uploads/FirstCard/${card.image}`}
              alt={card.image}
